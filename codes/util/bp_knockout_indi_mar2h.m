...
minKL_mar2h_all_L = zeros(length(subs_human), length(subs_marmoset), 105, length(fiber_interest));
minKL_mar2h_all_R = zeros(length(subs_human), length(subs_marmoset), 105, length(fiber_interest));
...
for h=1:length(subs_human)
    for m=1:length(subs_marmoset)
        ...
        bp_marmoset_l = load(strcat(resultpath, 'bp_atlas_indi/marmoset_MBM/', subs_marmoset{m}, '_BP_atlas.L.txt'));
        bp_marmoset_r = load(strcat(resultpath, 'bp_atlas_indi/marmoset_MBM/', subs_marmoset{m}, '_BP_atlas.R.txt'));
        ...
        for f=1:length(fiber_interest)-1
            ...
            bp_marmoset_l_loo(:, fiber_interest(f)) = [];
            
            bp_marmoset_r_loo = bp_marmoset_r;
            bp_marmoset_r_loo(:, fiber_interest(f)) = [];
            
            KL_L = calc_KL(bp_human_l_loo, bp_marmoset_l_loo);
            [minKL_mar2h_all_L(h, m, :, f), ~] = min(KL_L,[],2);
            
            KL_R = calc_KL(bp_human_r_loo, bp_marmoset_r_loo);
            [minKL_mar2h_all_R(h, m, :, f), ~] = min(KL_R,[],2);
        end
        
        % KL with ALL tracts
        KL_L = calc_KL(bp_human_l, bp_marmoset_l);
        [minKL_mar2h_all_L(h, m, :, length(fiber_interest)), ~] = min(KL_L,[],2);
        
        KL_R = calc_KL(bp_human_r, bp_marmoset_r);
        [minKL_mar2h_all_R(h, m, :, length(fiber_interest)), ~] = min(KL_R,[],2);
    end
end

save(strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_all_subject.L.mat'), 'minKL_mar2h_all_L');
save(strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_all_subject.R.mat'), 'minKL_mar2h_all_R');
...
for f=1:length(fiber_interest)
    ...
    minKL_mar2h_L = minKL_mar2h_L(:,f);
    minKL_mar2h_R = minKL_mar2h_R(:,f);
    ...
    save(temp_human_l, strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_', fibers{fiber_interest(f)}, '.L.func.gii'));
    save(temp_human_r, strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_', fibers{fiber_interest(f)}, '.R.func.gii'));

    save(strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_', fibers{fiber_interest(f)}, '.L.txt'), 'minKL_mar2h_L', '-ascii');
    save(strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_', fibers{fiber_interest(f)}, '.R.txt'), 'minKL_mar2h_R', '-ascii');
end


%% calculate deltaKL for each tract
deltaKL_mar2h_A45c_L = zeros(length(subs_human), length(subs_marmoset), length(fiber_interest)-1);
deltaKL_mar2h_A45c_R = zeros(length(subs_human), length(subs_marmoset), length(fiber_interest)-1);

for f=1:length(fiber_interest)-1
   
    deltaKL_mar2h_A45c_L(:,:,f) = abs(minKL_mar2h_all_L(:,:,17,f) - minKL_mar2h_all_L(:,:,17,5));
    deltaKL_mar2h_A45c_R(:,:,f) = abs(minKL_mar2h_all_R(:,:,17,f) - minKL_mar2h_all_R(:,:,17,5));
    
end

save(strcat(resultpath, 'knockout_indi/mar2h_deltaKL_on_human_atlas_A45c.L.mat'), 'deltaKL_mar2h_A45c_L');
save(strcat(resultpath, 'knockout_indi/mar2h_deltaKL_on_human_atlas_A45c.R.mat'), 'deltaKL_mar2h_A45c_R');

minKL_mar2h_A45c_L = squeeze(minKL_mar2h_all_L(:,:,17,:));
minKL_mar2h_A45c_R = squeeze(minKL_mar2h_all_R(:,:,17,:));

save(strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_A45c.L.mat'), 'minKL_mar2h_A45c_L');
save(strcat(resultpath, 'knockout_indi/mar2h_minKL_on_human_atlas_A45c.R.mat'), 'minKL_mar2h_A45c_R');
