...
minKL_c2h_all_L = zeros(length(subs_human), length(subs_chimp), 105);
minKL_c2h_all_R = zeros(length(subs_human), length(subs_chimp), 105);

for h=1:length(subs_human)
    for c=1:length(subs_chimp)
        disp(strcat(num2str(h), '-', num2str(c)));
        ...
        bp_chimp_l = load(strcat(resultpath, 'bp_atlas_indi/chimp/', subs_chimp{c}, '_BP_atlas.L.txt'));
        bp_chimp_r = load(strcat(resultpath, 'bp_atlas_indi/chimp/', subs_chimp{c}, '_BP_atlas.R.txt'));
        bp_chimp_l = bp_chimp_l(:,fiber_index_l);
        bp_chimp_r = bp_chimp_r(:,fiber_index_r);
        
        KL_L = calc_KL(bp_human_l, bp_chimp_l);
        [minKL_c2h_all_L(h, c, :), ~] = min(KL_L,[],2);
        
        KL_R = calc_KL(bp_human_r, bp_chimp_r);
        [minKL_c2h_all_R(h, c, :), ~] = min(KL_R,[],2);
...
save(temp_human_l, strcat(resultpath, 'minKL_indi/c2h_minKL_on_human_atlas.L.func.gii'));
save(temp_human_r, strcat(resultpath, 'minKL_indi/c2h_minKL_on_human_atlas.R.func.gii'));

save(strcat(resultpath, 'minKL_indi/c2h_minKL_on_human_atlas.L.txt'), 'minKL_c2h_L', '-ascii');
save(strcat(resultpath, 'minKL_indi/c2h_minKL_on_human_atlas.R.txt'), 'minKL_c2h_R', '-ascii');

save(strcat(resultpath, 'minKL_indi/c2h_minKL_on_human_atlas_all_subject.L.mat'), 'minKL_c2h_all_L');
save(strcat(resultpath, 'minKL_indi/c2h_minKL_on_human_atlas_all_subject.R.mat'), 'minKL_c2h_all_R');
