...
rootdir = '/n05dat/yfwang/backup_marmoset/marmoset_brain_minds/invivo/'
with open('/n05dat/yfwang/backup_marmoset/marmoset_brain_minds/list_t1_dwi_aging.txt', 'r') as f:
    sublist = [str(name).strip() for name in f.readlines()]

datapath = '/n05dat/yfwang/user/MarmosetWM/support_data/surf'
...
for sub in sublist:
    ...
    for surf in ['white', 'pial']:
        ...
        # command = '/n05dat/yfwang/user/software/antsbin/bin/antsApplyTransformsToPoints -d 3 -i {} -o {} -t [{}, 1] -t {}'.format(trans_template_path, trans_DTI_path, ants_mat_path, ants_warp_path)
        command = '/n05dat/yfwang/user/software/antsbin/bin/antsApplyTransformsToPoints -d 3 -i {} -o {} -t {} -t {}'.format(trans_template_path, trans_DTI_path, ants_warp_path, ants_mat_path)
        print(command)
        n = os.system(command)
        print(n)
