...
for s=1:length(subs)
    ...
    atlas = load_untouch_nii(strcat(resultpath, '/marmoset_MBM/', sub, '/', sub, '_BNA_in_b0.nii.gz'));

    af_l = load_untouch_nii(strcat(resultpath, '/marmoset_MBM/', sub, '/xtract/af_l/density.nii.gz'));
    af_r = load_untouch_nii(strcat(resultpath, '/marmoset_MBM/', sub, '/xtract/af_r/density.nii.gz'));

    seed_l = load_untouch_nii(strcat(resultpath, '/marmoset_MBM/', sub, '/masks/af_l/seed.nii.gz'));
    seed_r = load_untouch_nii(strcat(resultpath, '/marmoset_MBM/', sub, '/masks/af_r/seed.nii.gz'));
    ...
    save(temp_l, strcat(resultpath, '/marmoset_MBM/', sub, '/', sub, '_af_l_thr', num2str(thr), '_norm.func.gii'));
    save(temp_r, strcat(resultpath, '/marmoset_MBM/', sub, '/', sub, '_af_r_thr', num2str(thr), '_norm.func.gii'));
...
save(strcat(resultpath, '/result/marmoset_MBM_af_l_thr', num2str(thr), '_norm.txt'), 'count_norm_l', '-ascii');
save(strcat(resultpath, '/result/marmoset_MBM_af_r_thr', num2str(thr), '_norm.txt'), 'count_norm_r', '-ascii');

%% save to gifti
temp_l.cdata = zeros(size(temp_l.cdata));
temp_r.cdata = zeros(size(temp_r.cdata));
for i=1:length(subs)
    af_l = gifti(strcat(resultpath, '/marmoset_MBM/', subs{i}, '/', subs{i}, '_af_l_thr', num2str(thr), '_norm.func.gii'));
    af_r = gifti(strcat(resultpath, '/marmoset_MBM/', subs{i}, '/', subs{i}, '_af_r_thr', num2str(thr), '_norm.func.gii'));
...
save(temp_l, strcat(resultpath, '/result/marmoset_MBM_af_l_thr', num2str(thr), '_norm.func.gii'));
save(temp_r, strcat(resultpath, '/result/marmoset_MBM_af_r_thr', num2str(thr), '_norm.func.gii'));
