...
for s=1:length(subs)
    ...
    atlas = load_untouch_nii(strcat(resultpath, '/macaque_tvb/', sub, '/', sub, '_BNA_in_b0.nii.gz'));

    af_l = load_untouch_nii(strcat(resultpath, '/macaque_tvb/', sub, '/xtract/af_l/density.nii.gz'));
    af_r = load_untouch_nii(strcat(resultpath, '/macaque_tvb/', sub, '/xtract/af_r/density.nii.gz'));

    seed_l = load_untouch_nii(strcat(resultpath, '/macaque_tvb/', sub, '/masks/af_l/seed.nii.gz'));
    seed_r = load_untouch_nii(strcat(resultpath, '/macaque_tvb/', sub, '/masks/af_r/seed.nii.gz'));
    ...
    save(temp_l, strcat(resultpath, '/macaque_tvb/', sub, '/', sub, '_af_l_thr', num2str(thr), '_norm.func.gii'));
    save(temp_r, strcat(resultpath, '/macaque_tvb/', sub, '/', sub, '_af_r_thr', num2str(thr), '_norm.func.gii'));
...
save(strcat(resultpath, '/result/macaque_tvb_af_l_thr', num2str(thr), '_norm.txt'), 'count_norm_l', '-ascii');
save(strcat(resultpath, '/result/macaque_tvb_af_r_thr', num2str(thr), '_norm.txt'), 'count_norm_r', '-ascii');

%% save to gifti
temp_l.cdata = zeros(size(temp_l.cdata));
temp_r.cdata = zeros(size(temp_r.cdata));
for i=1:length(subs)
    af_l = gifti(strcat(resultpath, '/macaque_tvb/', subs{i}, '/', subs{i}, '_af_l_thr', num2str(thr), '_norm.func.gii'));
    af_r = gifti(strcat(resultpath, '/macaque_tvb/', subs{i}, '/', subs{i}, '_af_r_thr', num2str(thr), '_norm.func.gii'));
...
save(temp_l, strcat(resultpath, '/result/macaque_tvb_af_l_thr', num2str(thr), '_norm.func.gii'));
save(temp_r, strcat(resultpath, '/result/macaque_tvb_af_r_thr', num2str(thr), '_norm.func.gii'));
