...
## read fdata
df <- read.csv(file.path(WD, paste0("minKL_lesion_indi_", ROI, ".csv")), stringsAsFactors = TRUE)
...
df$tract       <- factor(df$tract,      levels = c("AF", "ATR", "SLF3", "UF", "ALL"))
df$human_id    <- factor(df$human_id)
df$nonhuman_id <- factor(df$nonhuman_id)

## MixedLM
fit <- lmer(minKL ~ species * tract * hemisphere + (1 | human_id) + (1 | nonhuman_id), data = df)
...
## 导出到 CSV
write.csv(df_emm,         file.path(WD, paste0("minKL_lesion_indi_", ROI, "_emm.csv")), row.names = FALSE)
write.csv(df_species,     file.path(WD, paste0("minKL_lesion_", ROI, "_emm_pairs_between_species.csv")), row.names = FALSE)
write.csv(df_hemispheres, file.path(WD, paste0("minKL_lesion_", ROI, "_emm_pairs_between_hemispheres.csv")), row.names = FALSE)
write.csv(df_tracts,      file.path(WD, paste0("minKL_lesion_", ROI, "_emm_pairs_between_tracts.csv")), row.names = FALSE)
write.csv(df_interaction, file.path(WD, paste0("minKL_lesion_", ROI, "_emm_interaction.csv")), row.names = FALSE)