...
arr=(`cat /gpfs/userdata/yfwang/Data/marmoset_MBM_preprocessed/marmoset_MBM_list.txt`)
sub=${arr[$SLURM_ARRAY_TASK_ID]}

datapath=/gpfs/userdata/yfwang/Data/marmoset_MBM_preprocessed/${sub}

protocolpath=/gpfs/userdata/yfwang/MarmosetWM/result/af_projection/protocol/marmoset
# copy from af_projection/protocol

resultpath=/gpfs/userdata/yfwang/MarmosetWM/result/af_projection/marmoset_MBM/${sub}
...
for tract in af_l af_r; do
    ...
    for angle in 30 45 60; do
        ...
        tckgen -stop -select 1k -angle ${angle} -maxlength 75 \
               ...
               -mask ${datapath}/DTI.bedpostX/nodif_brain_mask.nii.gz \
               ${datapath}/FOD/wmfod.mif \
    ...
    tckmap ${resultpath}/ifod2/${tract}/${tract}_forward.tck \
           ...
           -template ${datapath}/FOD/wmfod.mif \
           -force -nthread $NumofThreads
...
antsApplyTransforms -d 3 \
                    -i /n05dat/yfwang/user/MarmosetWM/support_data/MBM_v3.0.1/atlas_MBM_cortex_vPaxinos.nii.gz \
                    -o ${resultpath}/${sub}_Paxinos_in_b0.nii.gz \
                    -r ${datapath}/DTI/dti_FA.nii.gz \
                    -t [ ${datapath}/affine/ind2MBMv3/ind2MBMv30GenericAffine.mat, 1 ] \
                    -t ${datapath}/affine/ind2MBMv3/ind2MBMv31InverseWarp.nii.gz \
                    -n NearestNeighbor