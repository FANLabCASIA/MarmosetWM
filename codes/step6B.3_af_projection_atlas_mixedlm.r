...
## read fdata
df <- read.csv(file.path(WD, paste0("af_projection_", ROI, "_Paxinos.csv")), stringsAsFactors = TRUE)
...
## write to csv
write.csv(df_emm,         file.path(WD, paste0("af_projection_", ROI, "_Paxinos_emm.csv")), row.names = FALSE)
write.csv(df_species,     file.path(WD, paste0("af_projection_", ROI, "_Paxinos_emm_pairs_between_species.csv")), row.names = FALSE)
write.csv(df_hemispheres, file.path(WD, paste0("af_projection_", ROI, "_Paxinos_emm_pairs_between_hemispheres.csv")), row.names = FALSE)
write.csv(df_interaction, file.path(WD, paste0("af_projection_", ROI, "_Paxinos_emm_interaction.csv")), row.names = FALSE)
...
## read fdata
df <- read.csv(file.path(WD, paste0("af_projection_", ROI, "_Paxinos_marmoset.csv")), stringsAsFactors = TRUE)
...
## MixedLM
fit <- lmer(connectivity_score ~ atlas * hemisphere + (1 | subject_id), data = df)

## EMMs
emm <- emmeans(fit, ~ atlas * hemisphere)

# (1) compare between atlas in LH and RH（simple effects）
pairs_between_atlas <- pairs(emm, by = "hemisphere", adjust = "bonferroni")
print("---------- compare between atlas in LH and RH ----------")
print(pairs_between_atlas)

# (2) compare between hemispheres in every atlas effects）
pairs_between_hemispheres <- pairs(emm, by = "atlas", adjust = "bonferroni")
print("---------- compare between hemispheres in every atlas ----------")
print(pairs_between_hemispheres)

# (3) compare between atlas * hemispheres (interaction) 
inter_diff <- contrast(emm, interaction = "pairwise", adjust = "bonferroni")
print("---------- compare between atlas * hemispheres (interaction) ----------")
print(inter_diff)

# # (4) main effect of atlas (marginalize the hemispheres)
# emm_atlas <- emmeans(fit, ~ atlas)                 # 对 hemisphere 边际化
# pairs_atlas <- pairs(emm_atlas, adjust = "tukey")  # 经典物种主效应事后比较
# print(emm_atlas)
# print(pairs_atlas)

# # (5) 报告“半球主效应”（对 atlas 边际化）
# emm_hemisphere <- emmeans(fit, ~ hemisphere)
# pairs_hemisphere <- pairs(emm_hemisphere, adjust = "tukey")
# print(emm_hemisphere)
# print(pairs_hemisphere)
...
## write to csv
write.csv(df_emm,         file.path(WD, paste0("af_projection_", ROI, "_Paxinos_marmoset_emm.csv")), row.names = FALSE)
write.csv(df_atlas,       file.path(WD, paste0("af_projection_", ROI, "_Paxinos_marmoset_emm_pairs_between_species.csv")), row.names = FALSE)
write.csv(df_hemispheres, file.path(WD, paste0("af_projection_", ROI, "_Paxinos_marmoset_emm_pairs_between_hemispheres.csv")), row.names = FALSE)
write.csv(df_interaction, file.path(WD, paste0("af_projection_", ROI, "_Paxinos_marmoset_emm_interaction.csv")), row.names = FALSE)
