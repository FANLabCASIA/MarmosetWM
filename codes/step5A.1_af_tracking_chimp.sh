#!/bin/bash
#SBATCH --job-name=af_tracking_chimp
#SBATCH --output=log/af_tracking_chimp.%A_%a.out
#SBATCH --error=log/af_tracking_chimp.%A_%a.out
...
arr=(`cat /gpfs/userdata/yfwang/preprocess_fsl/chimp/chimplist46.txt`)
sub=${arr[$SLURM_ARRAY_TASK_ID]}

datapath_fsl=/gpfs/userdata/yfwang/preprocess_fsl/chimp/${sub}
datapath_mrtrix=/gpfs/userdata/yfwang/preprocess_mrtrix/chimp/${sub}

protocolpath=/gpfs/userdata/yfwang/MarmosetWM/result/af_projection/protocol/chimp
# copy from af_projection/protocol

resultpath=/gpfs/userdata/yfwang/MarmosetWM/result/af_projection/chimp/${sub}
    ...
    for roi in seed target1 target2 exclude stop; do
        ...
        if [ -f ${protocolpath}/${tract}/${roi}.nii.gz ]; then
            applywarp -i ${protocolpath}/${tract}/${roi}.nii.gz \
                      ...
                      -w ${datapath_fsl}/xfms/HCP_warps_2_DTI.nii.gz \
                      --interp=nn
        fi
    done
done
...
for tract in af_l af_r; do
    ...
    ${FSLDIR}/bin/probtrackx2_gpu10.2 -s ${datapath_fsl}/DTI.bedpostX/merged \
                                      ...
                                      --stop=${resultpath}/masks/${tract}/stop \
...
##############################
applywarp -i /gpfs/userdata/yfwang/MarmosetWM/support_data/atlas/chimp/atlas.nii.gz \
